...
int allocate_frame(pgtbl_entry_t *p) {
	...
	if(frame == -1) { // Didn't find a free page.
		...
		//if the address is not dirty, just change the PG_VALID and increment evict_clean_count
		if (!(coremap[frame].pte->frame & PG_DIRTY)){
			...
			// the address is already dirty
			//get the offset_number by swap_pageout and set the swap_off.
			
			int swap_offset = swap_pageout(frame , coremap[frame].pte->swap_off);
            coremap[frame].pte->swap_off = swap_offset;
            
            //if it is not ON_SWAP,change the value of it
            if (!(coremap[frame].pte->frame & PG_ONSWAP)){
				coremap[frame].pte->frame  ^= PG_ONSWAP;
			}
			
			//if it is VALID,change it become invalid
			if(coremap[frame].pte->frame  & PG_VALID){
				coremap[frame].pte->frame  ^= PG_VALID;
			}
			
			//if it is on disk, then it won't be dirty, and increment the evict_dirty_count
			if((coremap[frame].pte->frame  & PG_DIRTY)){
...
char *find_physpage(addr_t vaddr, char type) {
	...
	// Use top-level page directory to get pointer to 2nd-level page table
        ...
        pgtbl_entry_t *pgtbl = (pgtbl_entry_t *) ((pgdir[idx].pde >> 1) << 1);
	...
	// Use vaddr to get index into 2nd-level page table and initialize 'p'
       ...
       if (!(p->frame & PG_VALID )) {
		   ...
		   if (!(p->frame & PG_ONSWAP) ){
			   ...
			   //shift frame number to left, inorder to store the last four bit
			   p->frame = p->frame << PAGE_SHIFT;
			   p->frame = p->frame | PG_VALID |  PG_REF | PG_DIRTY ;
               
               //increment the miss count and ref count
			   miss_count +=1;
			   ref_count +=1;

		    }else{
				
				//on swap means it is on the disk, call swap_pagein 
				//init a new frame
			   int frame = allocate_frame(p);
			   swap_pagein(frame, p->swap_off);
			   
			   //keep the last four bit and set ref bit and valid bit
			   p->frame = (frame << PAGE_SHIFT)| (p->frame & (0xf));
			...
			// if it is valid then it is hit 
            hit_count += 1;
            ref_count += 1;
		}
		
		
		// if type is M or S, then it is dirty
		if (type=='M'||type=='S'){
            p->frame = p->frame | PG_DIRTY | PG_REF;
